        ...
        waitForLoaded(function () {
         ...
         var options = merge(merge(defaultOptions, chartOptions), chart.options.library || {});
          ...
          chart.chart = new google.visualization.PieChart(chart.element);
		   if(chart.options.enableClick) {
		   function selectHandler() {
			  var selectedItem = chart.chart.getSelection()[0];
			  if (selectedItem) {
				var deviceType = data.getValue(selectedItem.row, 0);
				 $.ajax({
					type : 'get',
					url : "/enforcements/reports",
					data : "type=advbytype&startYear="+$("#startYear").val()+"&mfr="+$("#mfr").val()+"&deviceType="+deviceType,    
					async : false,
